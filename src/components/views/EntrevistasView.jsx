...
import { Search, Plus, Trash2, Edit2, AlertCircle, Loader2, Users } from "lucide-react";
...
export default function EntrevistasView({ dependencias, entities, currentUser }) {
  ...
  const [editEntrevista, setEditEntrevista] = useState(null);
  ...
  const loadEntrevistas = async () => {
    ...
    try {
      const resp = await fetch(`${API_BASE_URL}/trd/entity/${activeEntityId}/entrevistas`, {
        headers: {
          "Authorization": `Bearer ${currentUser?.token || ''}`
        }
      });
      if (!resp.ok) throw new Error("Error al cargar entrevistas");
  ...
  const handleDelete = async (id) => {
    if (!confirm("¿Está seguro de eliminar esta entrevista?")) return;
    setDeletingId(id);
    try {
      const resp = await fetch(`${API_BASE_URL}/trd/entity/${activeEntityId}/entrevistas/${id}`, {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${currentUser?.token || ''}`
        }
      });
      if (!resp.ok) throw new Error("Error al eliminar entrevista");
  ...
  const filteredEntrevistas = entrevistas.filter(e => {
    ...
    const entrevistadoNom = e.entrevistado?.nombres?.toLowerCase() || "";
    const entrevistadoApe = e.entrevistado?.apellidos?.toLowerCase() || "";
    const cargo = e.entrevistado?.cargo?.toLowerCase() || "";
    const dep = dependencias.find(d => d.id === e.dependencia_id);
    const depName = dep?.nombre?.toLowerCase() || "";
    const fullName = `${entrevistadoNom} ${entrevistadoApe}`;
    
    return fullName.includes(term) || cargo.includes(term) || depName.includes(term);
  });
        ...
        <div className="max-w-6xl mx-auto">
          {loading ? (
            ...
            <div className="border border-dashed border-border rounded-xl p-12 text-center bg-card flex flex-col items-center justify-center min-h-[300px]">
              ...
              <h3 className="text-xl font-bold text-foreground">No hay entrevistas creadas</h3>
              <p className="text-muted-foreground mt-2 max-w-sm mb-6">
                No se encontraron entrevistas {searchQuery ? "que coincidan con tu búsqueda." : "registradas para esta entidad."}
              </p>
              {!searchQuery && (
                <button 
                  onClick={handleCreate}
                ...
                >
                  <Plus className="h-4 w-4" /> Registrar la primera entrevista
                </button>
              )}
            </div>
          ) : (
                  ...
                  <table className="w-full text-sm text-left">
                    <thead className="text-xs text-muted-foreground uppercase bg-secondary/50 border-b border-border font-semibold">
                      <tr>
                        ...
                        <th className="px-6 py-4 w-[35%]">Entrevistado</th>
                        <th className="px-6 py-4 w-[30%]">Dependencia</th>
                        ...
                        return (
                          <tr key={e.id} className="hover:bg-secondary/30 transition-colors">
                            <td className="px-6 py-4">
                              <span className="font-mono bg-secondary px-2 py-1 rounded text-xs font-medium border border-border text-foreground">
                                {e.fecha_entrevista || "S/F"}
                              </span>
                            </td>
                            <td className="px-6 py-4">
                              <div className="font-semibold text-foreground line-clamp-1">
                                {e.entrevistado?.nombres} {e.entrevistado?.apellidos}
                              </div>
                              <div className="text-xs text-muted-foreground mt-1 line-clamp-1">
                                {e.entrevistado?.cargo || "Sin cargo"}
                              </div>
                            </td>
                            <td className="px-6 py-4">
