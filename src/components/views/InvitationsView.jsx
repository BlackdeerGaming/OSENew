...
export default function InvitationsView({ currentUser, API_BASE_URL, onNavigate, entities = [] }) {
  ...
  const handleArchive = async (id, archived = true) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(err.detail || "Error al procesar el archivado");
      }
  ...
  const handleBulkArchive = async (archived = true) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json();
        alert(err.detail || "Error en archivado masivo");
      }
    } catch (e) { console.error(e); }
  ...
  const handleCleanHistory = async () => {
    const eligibleIds = invitations
      .filter(inv => ['aceptada', 'rechazada', 'vencida', 'cancelada'].includes(inv.status))
      .map(inv => inv.id);
      
    if (eligibleIds.length === 0) {
      alert("No hay invitaciones inactivas para limpiar en este momento.");
      return;
    }
    
    if (!window.confirm(`Esta acción archivará automáticamente ${eligibleIds.length} invitaciones terminadas. ¿Continuar?`)) return;
              ...
              <div className="flex flex-col gap-6 mb-8">
                <div className="flex flex-wrap items-center justify-between gap-4">
                   <div className="flex bg-slate-200/50 p-1 rounded-2xl border border-slate-200 shadow-sm">
                      <button 
                        ...
                        className={cn(
                          ...
                          !filterArchived ? "bg-white text-primary shadow-md" : "text-slate-500 hover:text-slate-700"
                        )}
                      >
                        Activas
                      </button>
                   ...
                   <div className="flex items-center gap-3">
                      {selectedIds.size > 0 && (
                        <button 
                          onClick={() => handleBulkArchive(!filterArchived)}
                          disabled={isArchiving}
                          className="px-6 py-3 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-slate-900/10 hover:scale-[1.02] transition-all flex items-center gap-2"
                        >
                          {isArchiving ? <Loader2 className="h-3 w-3 animate-spin" /> : <Shield className="h-3.5 w-3.5" />}
                          {filterArchived ? 'Restaurar Seleccionadas' : 'Archivar Seleccionadas'}
                        </button>
                      )}
                      ...
                      {!filterArchived && (
                        <button 
                          ...
                          disabled={isArchiving}
                          className="px-6 py-3 bg-white border-2 border-slate-100 text-slate-500 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:border-primary/20 hover:text-primary transition-all flex items-center gap-2"
                        >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {activeTab === 'sent' && (
                           <div className="flex items-center gap-1">
                             ...
                             {inv.status !== 'aceptada' && isInvitationAdmin(inv.entity_id) && !filterArchived && (
                               <button
                                 disabled={processingId === inv.id}
                             ...
                             {inv.status === 'pendiente' && !filterArchived && (
                               <button 
                                 disabled={processingId === inv.id}
                             ...
                             {isInvitationAdmin(inv.entity_id) && (
                               <button 
                                 disabled={processingId === inv.id}
                                 onClick={() => handleArchive(inv.id, !filterArchived)}
                                 className={cn(
                                   "p-3 rounded-xl transition-all",
                                   filterArchived ? "text-primary hover:bg-primary/5" : "text-slate-400 hover:text-slate-600 hover:bg-slate-100"
                                 )}
