...
export default function FuncionesView({ dependencias, entities, currentUser }) {
  ...
  const loadFunciones = async () => {
    ...
    setError(null);
    try {
      const resp = await fetch(`${API_BASE_URL}/trd/entity/${currentUser.entity_id}/funciones`, {
        headers: {
  ...
  const handleDelete = async (id) => {
    ...
    setDeletingId(id);
    try {
      const resp = await fetch(`${API_BASE_URL}/trd/entity/${currentUser.entity_id}/funciones/${id}`, {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${currentUser.token || ''}`
        }
        ...
        <div className="max-w-6xl mx-auto flex items-end justify-between gap-4">
          <div className="flex-1 space-y-1 relative pl-12">
            ...
            <p className="text-sm text-muted-foreground flex items-center gap-2">
              Gestión de funciones administrativas por dependencia
            </p>
          </div>
          <div className="flex items-center gap-3">
            ...
            >
              <Plus className="h-4 w-4" /> Crear Función
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-6xl mx-auto">
          {loading ? (
            ...
            <div className="border border-dashed border-border rounded-xl p-12 text-center bg-card flex flex-col items-center justify-center min-h-[300px]">
              ...
              <h3 className="text-xl font-bold text-foreground">No hay funciones</h3>
